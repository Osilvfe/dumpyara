...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    if [[ "$(command -v apt)" != "" ]]; then
        $sudo_cmd apt install unace unrar zip unzip p7zip-full p7zip-rar sharutils rar uudeview mpack arj cabextract device-tree-compiler liblzma-dev python3-pip brotli build-essential python3-dev liblz4-tool axel gawk aria2 detox cpio rename liblz4-dev curl python3-venv -y
...
"$PIP" install aospdtgen backports.lzma extract-dtb protobuf Crypto pycryptodome docopt zstandard pycryptodomex
